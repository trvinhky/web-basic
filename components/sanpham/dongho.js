...
function createCartProduct(data) {
  ...
  price.innerHTML = `${convertPrice(data.price)}<span>đ</span>`;
  bottom.appendChild(price);
...
function initViewProduct() {
  const locaHref = location.href;
  const dataObj = Object.entries(objProduct);
  if (locaHref.indexOf("donghonam") !== -1) {
    dataOfProduct = dataObj.filter((item) => item[1].gender === 1);
  } else if (locaHref.indexOf("donghonu") !== -1) {
    dataOfProduct = dataObj.filter((item) => item[1].gender === 0);
  } else {
    dataOfProduct = dataObj.filter((item) => item[1].gender === 2);
  }
  dataOfProduct = dataOfProduct.map((item) => ({
    code: item[0],
    ...item[1],
  }));
...
function renderProduct(data) {
  ...
  const likes = JSON.parse(localStorage.getItem("likes"));
  if (likes) renderViewHeart(likes);
}
